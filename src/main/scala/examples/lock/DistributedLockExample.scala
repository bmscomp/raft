...
/** Example 2: Distributed Lock Service
  *
  * A distributed lock service replicated via RAFT. Similar to ZooKeeper's lock
  * recipes or etcd's distributed locks.
  */
/** Commands for acquiring, releasing, and renewing distributed locks.
  *
  * Each command is serialized to a byte array for inclusion in the RAFT log.
  */
...
/** Metadata tracked for each active distributed lock.
  *
  * @param owner
  *   client identifier that currently holds the lock
  * @param expiresAt
  *   absolute timestamp (epoch millis) when the lock expires
  */
...
/** In-memory lock state machine implementing [[raft.spi.StateMachine]].
  *
  * Manages distributed locks with TTL-based expiration and owner verification.
  * Supports acquire, release, and heartbeat renewal operations, similar to
  * ZooKeeper lock recipes or etcd distributed locks.
  *
  * @param locks
  *   atomic reference to the map of active locks
  */
...
class LockStateMachine[F[_]: Sync: Clock](
  ...
  def snapshot: F[Array[Byte]] =
    locks.get.map { m =>
      m.map { case (k, v) => s"$k:${v.owner}:${v.expiresAt}" }
        .mkString("\n")
        .getBytes
    }
