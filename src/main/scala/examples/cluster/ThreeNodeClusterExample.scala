 ...
 * 
 * Demonstrates setting up a complete 3-node RAFT cluster
 * using pure logic functions.
 */

// === Simple StateMachine for cluster demo ===
...
object ThreeNodeClusterExample extends IOApp.Simple:
  ...
  private def runSimulation: IO[Unit] = IO {
    ...
    var node1State: NodeState = NodeState.Follower(term = 0, votedFor = None, leaderId = None)
    var node2State: NodeState = NodeState.Follower(term = 0, votedFor = None, leaderId = None)
    var node3State: NodeState = NodeState.Follower(term = 0, votedFor = None, leaderId = None)
    ...
    println(s"   Effects: ${transition1.effects.map(e => e.getClass.getSimpleName)}")
    ...
    println(s"   Node-2 effects: ${trans2.effects.map(e => e.getClass.getSimpleName)}")
    println(s"   Node-3 state: $node3State")
    println(s"   Node-3 effects: ${trans3.effects.map(e => e.getClass.getSimpleName)}")
    ...
    println(s"   Leader heartbeat: term=${heartbeat.term}, leader=${heartbeat.leaderId.value}")
