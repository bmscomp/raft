...
/** ════════════════════════════════════════════════════════════════════════════
  ...
  * == Transaction Model ==
  *
  * We implement a multi-key transaction system with:
  ...
  * 2. Leader validates preconditions (for CAS operations)
  * 3. Transaction is appended to RAFT log
  * 4. Once committed (majority acknowledged), transaction is applied
...
object DistributedTransactionExample extends IOApp.Simple:
  ...
  /** Unique transaction identifier for tracking and idempotency. */
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // TRANSACTION SERIALIZATION (for RAFT log)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  object Transaction:
    ...
    def encode(txn: Transaction): Array[Byte] =
      val ops = txn.operations.map {
        ...
        case TxnOperation.CompareAndSet(k, e, n) => s"CAS$FIELD_SEP$k$FIELD_SEP${e.getOrElse("NULL")}$FIELD_SEP$n"
    ...
    private def parseOperations(str: String): Either[String, List[TxnOperation]] =
  ...
  /** State machine that executes transactions atomically.
    *
    * Each transaction is validated and applied as an atomic unit.
    * Failed preconditions (CAS mismatches) abort the entire transaction.
    */
  class TransactionalStore[F[_]: Sync](
    ...
    def apply(log: Log): F[TxnResult] =
      Transaction.decode(log.data) match
        case Left(err) => 
          Sync[F].pure(TxnResult.Aborted(TxnId("unknown"), s"Decode error: $err"))
        
        case Right(txn) =>
          executeTransaction(txn, log.index)
    
    /** Execute transaction with precondition checking. */
    private def executeTransaction(txn: Transaction, logIndex: Long): F[TxnResult] =
      for
    ...
    private def updateVersions(ops: List[TxnOperation], version: Long): F[Unit] =
      val keys = ops.map {
      ...
      versions.update(v => keys.foldLeft(v)((acc, k) => acc.updated(k, version)))
  ...
  private def runDemo: IO[Unit] =
    for
      ...
      _ <- IO.println(s"   State: A=${accountA.getOrElse("?")} B=${accountB.getOrElse("?")}\n")
      ...
      txn3 = Transaction(
        id = TxnId.generate,
        operations = List(
          TxnOperation.CompareAndSet("account:A", Some("900"), "800")  // 900 is stale!
        )
      )
      ...
      _ <- IO.println(s"   account:A = ${accountA3.getOrElse("?")} (unchanged)\n")
      
      // ─────────────────────────────────────────────────────────────────────
      _ <- IO.println("Example 4: Atomic Counter Increment")
      _ <- IO.println("─" * 50)
      _ <- IO.println("Increment a counter atomically (no read-modify-write race).\n")
      ...
      _ <- IO.println("─" * 50)
      _ <- IO.println("Complex transaction: create user + set permissions + log event.\n")
      
      txn5 = Transaction(
        id = TxnId.generate,
        operations = List(
          TxnOperation.Put("user:alice", "{name:Alice,email:alice@example.com}"),
      ...
      _ <- txnLog.traverse_ { result =>
        result match
          ...
          case TxnResult.ConflictDetected(id, key, exp, act) =>
            IO.println(s"   ⚠ ${id.value} CONFLICT on $key: expected=$exp, actual=$act")
      }
