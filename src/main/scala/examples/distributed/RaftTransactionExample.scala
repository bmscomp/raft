...
/** ════════════════════════════════════════════════════════════════════════════
  * DISTRIBUTED TRANSACTIONS WITH RAFT - Full Library Integration
  * ════════════════════════════════════════════════════════════════════════════
  *
  * This example demonstrates distributed transactions using the RAFT library's
  * core components: RaftLogic for pure state transitions, effects for I/O, and
  * a TransactionalStore state machine.
  *
  * == Architecture ==
  *
  *   Client → RaftNode (Leader) → RaftLogic → Effects → Replication
  *                                    ↓
  *                            TransactionalStore.apply()
  *
  * == RAFT Library Components Used ==
  *
  * - RaftLogic: Pure state transitions for consensus
  * - NodeState: Follower/Candidate/Leader states
  * - RaftMessage: Protocol messages (AppendEntries, RequestVote)
  * - Effect: Side effects (SendMessage, AppendLogs, CommitEntries)
  * - RaftConfig: Node configuration
  * - StateMachine: Transaction execution
  */
object RaftTransactionExample extends IOApp.Simple:
  
  // ═══════════════════════════════════════════════════════════════════════════
  // TRANSACTION TYPES (same as before)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  enum TxnResult:
    ...
    case ConflictDetected(txnId: TxnId, key: String, expected: Option[String], actual: Option[String])
  ...
  object Transaction:
    ...
    def encode(txn: Transaction): Array[Byte] =
      val ops = txn.operations.map {
        ...
        case TxnOperation.CompareAndSet(k, e, n) => s"CAS$SEP$k$SEP${e.getOrElse("NULL")}$SEP$n"
    ...
    private def parseOps(str: String): Either[String, List[TxnOperation]] =
      ...
      else str.split(OP_SEP).toList.traverse { op =>
        op.split(SEP).toList match
          case "PUT" :: k :: v :: Nil => Right(TxnOperation.Put(k, v))
          case "DEL" :: k :: Nil => Right(TxnOperation.Delete(k))
          case "CAS" :: k :: e :: n :: Nil =>
            Right(TxnOperation.CompareAndSet(k, if e == "NULL" then None else Some(e), n))
          case "INC" :: k :: d :: Nil => Right(TxnOperation.Increment(k, d.toLong))
          case _ => Left(s"Invalid op: $op")
      }
  
  // ═══════════════════════════════════════════════════════════════════════════
  // TRANSACTIONAL STATE MACHINE (implements StateMachine SPI)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  class TransactionalStore[F[_]: Sync](
    ...
    private def validate(txn: Transaction, current: Map[String, String]): Option[TxnResult] =
    ...
    private def applyOps(ops: List[TxnOperation]): F[Unit] =
      ops.traverse_ {
        ...
        case TxnOperation.CompareAndSet(k, _, v) => state.update(_.updated(k, v))
        case TxnOperation.Increment(k, d) => state.update { s =>
    ...
    def snapshot: F[Array[Byte]] = state.get.map(_.map((k,v) => s"$k=$v").mkString("\n").getBytes)
  ...
  // Uses RaftLogic for consensus, StateMachine for transaction execution
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  class RaftTransactionNode[F[_]: Async](
        ...
        result <- state match
          case l: Leader =>
            for
              ...
              _ <- Async[F].delay(println(s"   📝 [${nodeId.value}] Appended txn ${txn.id.value} at index $newIndex"))
    ...
    def processMessage(from: NodeId, msg: RaftMessage): F[Unit] =
      for
        ...
        transition = RaftLogic.onMessage(state, msg, config, lastIdx, lastTerm, clusterSize, getTermAt)
    ...
    private def executeEffect(effect: raft.effect.Effect): F[Unit] =
      effect match
        ...
        case Broadcast(msg) =>
          Async[F].delay(println(s"   📡 [${nodeId.value}] Broadcasting ${msg.getClass.getSimpleName}"))
        
        case AppendLogs(entries) =>
          logRef.update(_ ++ entries) *>
          Async[F].delay(println(s"   📥 [${nodeId.value}] Appended ${entries.size} entries"))
        ...
        case CommitEntries(upToIndex) =>
          for
            ...
            _ <- toApply.traverse_ { entry =>
              ...
              Async[F].delay(println(s"   ✅ [${nodeId.value}] Applied entry ${entry.index}"))
            }
    ...
    private def broadcastToFollowers(msg: RaftMessage): F[Unit] =
      Async[F].delay(println(s"   📡 [${nodeId.value}] Replicating to followers..."))
  ...
  object RaftTransactionNode:
    def apply[F[_]: Async](
      ...
      yield new RaftTransactionNode[F](nodeId, config, state, log, commit, stateMachine, queue, clusterSize)
  ...
  private def runDemo: IO[Unit] =
    for
      ...
      voteReq = RequestVoteRequest(term = 1, candidateId = NodeId("node-1"), lastLogIndex = 0, lastLogTerm = 0)
      ...
      _ <- log1.traverse_ { entry =>
        Transaction.decode(entry.data) match
          case Right(txn) => IO.println(s"      Index ${entry.index}: txn ${txn.id.value} with ${txn.operations.size} ops")
          case Left(_) => IO.unit
      }
      ...
      _ <- IO.println(s"\n   Transaction log:")
      _ <- txnLog.traverse_ {
        case TxnResult.Committed(id, v) => IO.println(s"      ✓ ${id.value} committed at version $v")
        case TxnResult.ConflictDetected(id, k, _, _) => IO.println(s"      ⚠ ${id.value} conflict on $k")
        case TxnResult.Aborted(id, r) => IO.println(s"      ✗ ${id.value} aborted: $r")
      }
