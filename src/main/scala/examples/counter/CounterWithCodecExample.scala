...
/** Example 3: Distributed Counter with Custom Message Codec
  ...
  */
/** Counter commands with parametric increment and decrement amounts.
  *
  * Encoded as simple text strings for RAFT log entries.
  */
...
/** In-memory counter state machine implementing [[raft.spi.StateMachine]].
  *
  * Returns the updated counter value (`Long`) after each command application.
  *
  * @param counter
  *   atomic reference to the current counter value
  */
...
/** Simple text-based wire format used to demonstrate a custom `MessageCodec`.
  *
  * @param content
  *   pipe-delimited string encoding of a RAFT message
  */
