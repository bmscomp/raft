...
/** Example demonstrating log matching validation in RAFT.
  ...
  * log doesn't match the leader's prevLogIndex/prevLogTerm, ensuring
  * consistency.
  */
object LogMatchingExample extends IOApp.Simple:
  ...
  /** Runs through four log matching scenarios:
    *   1. Empty log (always accepts)
    *   1. Matching prevLogIndex/prevLogTerm (accepts)
    *   1. Log too short — missing entry (rejects)
    *   1. Term mismatch — conflicting entry (rejects, triggers truncation)
    */
  private def demonstrateLogMatching(): Unit =
    println(
      "RAFT requires log consistency: before accepting entries, a follower"
    )
    ...
    println(
      "Follower has entry at index 5 with term 2, leader sends prevLogIndex=5, prevLogTerm=2.\n"
    )

    val matchingFollower =
      Follower(term = 3, votedFor = None, leaderId = Some(leaderId))
    ...
    println(
      s"  Result: success=${response2.success}, matchIndex=${response2.matchIndex} ✓"
    )
    ...
    println(
      "Leader thinks follower has 100 entries, but follower only has 50.\n"
    )
    ...
    println(
      "Follower has entry at index 10 with term 3, but leader has term 4 there.\n"
    )
    ...
    println(
      s"  The follower must truncate conflicting entries before accepting."
    )
  ...
  /** Extracts the first `AppendEntriesResponse` from a list of effects.
    *
    * @param effects
    *   list of effects produced by `RaftLogic.onMessage`
    * @return
    *   the `AppendEntriesResponse` or throws if none found
    */
  private def extractResponse(
      effects: List[raft.effect.Effect]
  ): AppendEntriesResponse =
    effects
      .collectFirst { case SendMessage(_, resp: AppendEntriesResponse) =>
        resp
      }
