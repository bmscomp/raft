...
/** Example demonstrating the TimerService for RAFT election and heartbeat
  * timing.
  *
  * Shows how to use cancellable timers with randomized jitter to prevent split
  * votes and ensure leader stability.
  */
object TimerServiceExample extends IOApp.Simple:
  ...
  /** Demonstrates timer creation, randomized jitter, scheduling, cancellation,
    * and heartbeat usage patterns with `DefaultTimerService`.
    */
  private def demonstrateTimers: IO[Unit] =
    for
      _ <- IO.println(
        "The TimerService provides cancellable timers with jitter for RAFT."
      )
      _ <- IO.println(
        "This prevents split votes by randomizing election timeouts.\n"
      )
      ...
      _ <- IO.println(
        "RAFT uses random timeouts (150-300ms typically) to reduce split votes.\n"
      )
      ...
      _ <- IO.println(
        "Timers are cancellable Fibers that fire after the specified delay.\n"
      )
      ...
      _ <- IO.println("\n━━━ Timer Cancellation ━━━")
      _ <- IO.println(
        "When a follower receives a valid heartbeat, it cancels and resets its timer.\n"
      )

      cancelCounter <- Ref.of[IO, Int](0)
      _ <- IO.println("  Scheduling election timer for 200ms...")

      cancelFiber <- timer.scheduleElection(
        200.millis,
        cancelCounter.update(_ + 1)
      )
      _ <- IO.sleep(50.millis)
      _ <- IO.println(
        "  Cancelling timer after 50ms (simulating heartbeat receipt)..."
      )
      ...
      _ <- IO.println(
        "Leaders send heartbeats at regular intervals (e.g., 50ms).\n"
      )

      heartbeatCounter <- Ref.of[IO, Int](0)
      _ <- IO.println(
        "  Scheduling 3 consecutive heartbeat timers (50ms each)..."
      )
  ...
  """.stripMargin)
      ...
      _ <- IO.println("\nKey takeaways:")
      _ <- IO.println(
        "  • Randomized timeouts prevent split votes in elections"
      )
      _ <- IO.println(
        "  • Timers return Fibers that can be cancelled on heartbeat"
      )
      _ <- IO.println(
        "  • Leaders use heartbeat timers, followers use election timers"
      )
      _ <- IO.println("  • Jitter is typically 50% of the base timeout")
    yield ()

  /** Schedules consecutive heartbeat timers, waiting for each to fire.
    *
    * @param timer
    *   the timer service to schedule heartbeats on
    * @param counter
    *   atomic counter incremented by each heartbeat callback
    * @param remaining
    *   number of heartbeats still to schedule
    */
