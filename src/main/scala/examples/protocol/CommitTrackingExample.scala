...
object CommitTrackingExample extends IOApp.Simple:
  ...
  /** Walks through the commit index lifecycle step by step.
    *
    * Demonstrates leader initialization, matchIndex tracking, majority-based
    * commit index calculation, heartbeat broadcasting, and follower commit
    * application — all using `RaftLogic.onMessage` directly.
    */
  private def demonstrateCommitTracking(): Unit =
    println(
      "RAFT commits entries when they are replicated to a majority of nodes."
    )
    println(
      "The leader tracks matchIndex for each follower and calculates commitIndex.\n"
    )

    // Step 1: Leader initializes indices
    println("━━━ Step 1: Leader Initialization ━━━")
    println(
      "When a node becomes leader, it initializes tracking for all followers.\n"
    )
    ...
    for (f, idx) <- initializedLeader.nextIndex do
      println(
        s"    ${f.value}: nextIndex=$idx, matchIndex=${initializedLeader.matchIndex(f)}"
      )
    ...
    for (f, idx) <- leader4.matchIndex do println(s"    ${f.value}: $idx")
    ...
    println(
      s"  matchIndex values: ${leader4.matchIndex.values.toList.sorted.reverse}"
    )
    ...
    heartbeat match
      case Some(hb) =>
        ...
        println(
          s"  Followers will commit entries up to index ${hb.leaderCommit}"
        )
      case None =>
    ...
    val follower =
      Follower(term = 5, votedFor = None, leaderId = Some(leaderId))
    ...
    commitEffect match
      case Some(idx) =>
        ...
        println(
          s"  Follower will apply entries 1 through $idx to state machine"
        )
      case None =>
    ...
    println("\nKey takeaways:")
    println(
      "  • Leader initializes nextIndex=lastLog+1, matchIndex=0 for all followers"
    )
    println("  • matchIndex updated when follower acknowledges replication")
    println(
      "  • commitIndex = median of matchIndex values (majority threshold)"
    )
