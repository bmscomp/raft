...
object PreVoteExample extends IOApp.Simple:
  ...
  private def demonstratePreVoteFlow: IO[Unit] =
    for
      ...
      trans1 = RaftLogic.onMessage(follower, ElectionTimeout, configWithPreVote, 10, 5, 3)
      ...
      preVoteReq = trans1.effects.collectFirst { case Broadcast(r: RequestVoteRequest) => r }
      _ <- IO.println(s"   Pre-vote request term: ${preVoteReq.get.term} (asking for NEXT term)")
      ...
      preVoteResp = RequestVoteResponse(term = 6, voteGranted = true, isPreVote = true)
      _ <- IO.println(s"   Received pre-vote grant from node-2")
      
      trans2 = RaftLogic.onMessage(trans1.state, preVoteResp, configWithPreVote, 10, 5, 3)
      _ <- IO.println(s"   After 1st pre-vote: ${trans2.state}")
      
      trans3 = RaftLogic.onMessage(trans2.state, preVoteResp, configWithPreVote, 10, 5, 3)
  ...
  private def compareWithoutPreVote: IO[Unit] =
    for
      ...
      follower = Follower(term = 5, votedFor = None, leaderId = None)
      trans = RaftLogic.onMessage(follower, ElectionTimeout, configNoPreVote, 10, 5, 3)
      
      _ <- IO.println(s"   With preVoteEnabled=false:")
      _ <- IO.println(s"   After ElectionTimeout: ${trans.state}")
      _ <- IO.println(s"   Immediately becomes Candidate at term ${trans.state.term}")
  ...
  private def simulatePartitionedNode: IO[Unit] =
    for
      ...
      trans1 = RaftLogic.onMessage(partitionedNode, ElectionTimeout, configWithPreVote, 0, 0, 3)
      _ <- IO.println(s"      Node becomes: ${trans1.state}")
      _ <- IO.println(s"      Term: ${trans1.state.term} (not incremented)")
      _ <- IO.println(s"      Cannot get pre-vote majority → stays PreCandidate")
      ...
      t1 = RaftLogic.onMessage(partitionedNode, ElectionTimeout, configNoPreVote, 0, 0, 3)
      _ <- IO.println(s"      Timeout 1: term=${t1.state.term}")
      t2 = RaftLogic.onMessage(t1.state, ElectionTimeout, configNoPreVote, 0, 0, 3)
      _ <- IO.println(s"      Timeout 2: term=${t2.state.term}")
      t3 = RaftLogic.onMessage(t2.state, ElectionTimeout, configNoPreVote, 0, 0, 3)
      _ <- IO.println(s"      Timeout 3: term=${t3.state.term}")
      _ <- IO.println(s"      When partition heals: high term causes leader step-down!")
