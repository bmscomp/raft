...
/**
 * JSON-based MessageCodec implementation.
 * 
 * Uses a simple JSON format for wire messages.
 * Suitable for debugging and interoperability.
 * 
 * Implements enhanced SimpleMessageCodec with typed errors.
 */
object JsonCodec extends SimpleMessageCodec[String]:
  ...
  def encode(msg: RaftMessage): String = msg match
    ...
    case InstallSnapshotRequest(term, leader, lastIdx, lastTerm, offset, data, done) =>
      ...
      json.get("type") match
        case Some("AE_REQ") =>
          for
            ...
            leader <- requireField[String](json, "leader", "AE_REQ").map(NodeId(_))
          ...
          yield AppendEntriesRequest(term, leader, prev, prevTerm, entries, commit)
        ...
        case Some("RV_REQ") =>
          for
            ...
            candidate <- requireField[String](json, "candidate", "RV_REQ").map(NodeId(_))
          ...
          yield RequestVoteRequest(term, candidate, lastIndex, lastTerm, preVote)
        ...
        case Some("IS_REQ") =>
          for
            ...
            leader <- requireField[String](json, "leader", "IS_REQ").map(NodeId(_))
          ...
          yield 
            ...
            InstallSnapshotRequest(term, leader, lastIndex, lastTerm, offset, data, done)
        ...
        case Some("TL_REQ") =>
          for
            ...
            target <- requireField[String](json, "target", "TL_REQ").map(NodeId(_))
          yield TransferLeadershipRequest(term, target)
        
        case Some("RI_REQ") =>
          for
            requestId <- requireField[String](json, "requestId", "RI_REQ")
        ...
        case Some("AS_REQ") =>
          for
            ...
            server <- requireField[String](json, "server", "AS_REQ").map(NodeId(_))
        ...
        case Some("RS_REQ") =>
          for
            ...
            server <- requireField[String](json, "server", "RS_REQ").map(NodeId(_))
  ...
  private def requireField[T](json: Map[String, String], name: String, msgType: String)(
  ...
  private def decodeLog(logStr: String): Log =
    ...
    val data = if dataB64.isEmpty then Array.emptyByteArray 
